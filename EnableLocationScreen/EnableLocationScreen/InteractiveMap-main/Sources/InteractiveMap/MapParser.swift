...
public class MapParser : NSObject, XMLParserDelegate {
    ...
    public func computeBounds() {
        ...
        // Sort pathDatas so that the largest area shapes are at the start and smallest at the end.
        // This ensures smaller states (like Delhi) are rendered on top of larger states (like UP or Haryana),
        // preventing their touch targets from being fully occluded.
        pathDatas.sort { p1, p2 in
            let area1 = (p1.boundingBox?.width ?? 0) * (p1.boundingBox?.height ?? 0)
            let area2 = (p2.boundingBox?.width ?? 0) * (p2.boundingBox?.height ?? 0)
            return area1 > area2
        }
    }
}
